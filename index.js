...
function () {
  function NotFoundTemplate() {
    ...
    _defineProperty(this, "template", function () {
      return React__default.createElement("div", null, "Not found!!");
    });
  }
...
var NotFound = new NotFoundTemplate();
...
var ServiceStore = function ServiceStore() {
  var _this = this;
  ...
  _defineProperty(this, "create", function (Service) {
    _this.store[Service] = new Service();
    return _this.store[Service];
  });

  _defineProperty(this, "get", function (Service) {
    if (_this.store[Service]) {
      return _this.store[Service];
    } else return _this.create(Service);
  });
};
  ...
  return function (Target) {
    ...
    function (_Target) {
      ...
      function Result(config) {
        ...
        _this = _possibleConstructorReturn(this, _getPrototypeOf(Result).call(this, config));

        _defineProperty(_assertThisInitialized(_assertThisInitialized(_this)), "models", {});

        var _loop = function _loop(i) {
          _this.models[i] = function (e) {
            _this[i] = e.target.value;
          };
        };

        for (var i in _assertThisInitialized(_assertThisInitialized(_this))) {
          _loop(i);
        }
...
var LayoutService = (_class = (_temp =
/*#__PURE__*/
function () {
...
var injectService = serviceStore.get;
var useService = function useService(Service) {
  return React.useMemo(function () {
    return injectService(Service);
  }, [Service]);
};
var service = ServiceDecorator;
var LayoutService$1 = LayoutService;

function runOnEnter(onEnter) {
  if (onEnter) {
    if (onEnter.length) for (var i in onEnter) {
      onEnter[i]();
    } else onEnter();
  }
}
...
function runOnOut(onOut) {
  if (onOut) {
    if (onOut.length) for (var i in onOut) {
      onOut[i]();
    } else onOut();
  }
}
...
function runProtect(protect) {
  if (protect) {
    if (protect.length) for (var i in protect) {
      var protectRes = protect[i]();

      if (protectRes) {
        return protectRes;
      }
    } else {
      var _protectRes = protect();
...
function createRouteComponent(opt) {
  ...
  var services = [];
  if (opt.disableLayout) services = !opt.disableLayout.length ? [injectService(opt.disableLayout)] : opt.disableLayout.map(function (item) {
    return injectService(item);
  });
  var isDisableLayouRun = false;

  var routedComponent = function routedComponent() {
    ...
    var isProtected = runProtect(opt.guard);
...
function createRouter(router, config) {
  ...
  var ResultComponent = function ResultComponent() {
    return React__default.createElement(reactRouterDom.Switch, null, router.map(function (item, index) {
      ...
      path: "*",
      component: Default
    }) : null, React__default.createElement(reactRouterDom.Route, {
      path: "*",
